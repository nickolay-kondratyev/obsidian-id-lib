...
function setup(): Setup {
  ...
  const store = new FrontmatterDocIdStore(fileAccess, new FixedDocIdGenerator(GENERATED_ID));
...
describe('FrontmatterDocIdStore', () => {
  describe('getId', () => {
    it('should return the existing frontmatter id without any write', async () => {
      ...
      expect({ id, writes: fileAccess.processCallCount })
  ...
  describe('ensureId', () => {
    ...
    it('should create a frontmatter block with the id when the note has none', async () => {
      ...
      const { store, fileAccess } = setup();
      const file = fileAccess.seedNote('notes/a.md', '# Just a heading');
      // WHEN
      const id = await store.ensureId(file);
    ...
    it('should preserve existing frontmatter formatting byte-for-byte when adding an id (quoted keys stay quoted)', async () => {
      ...
      expect(fileAccess.getContent('notes/a.md'))
        .toBe(`---\nid: ${GENERATED_ID}\n"someother field 3": v3\n---\nbody`);
    });
    ...
    it('should populate a degenerate empty frontmatter block', async () => {
      ...
      await store.ensureId(file);
      // THEN
      expect(fileAccess.getContent('notes/a.md')).toBe(`---\nid: ${GENERATED_ID}\n---\nbody`);
    });
    ...
    it('should ignore an id-looking line in the body when there is no frontmatter', async () => {
      ...
      const { store, fileAccess } = setup();
      const file = fileAccess.seedNote('notes/a.md', 'id: fake-body-id\ntext');
      // WHEN
      const id = await store.ensureId(file);
    ...
    it('should return the concurrent id when the content gains one between precheck and atomic write (re-check backstop)', async () => {
      // GIVEN a note without an id whose content gains one right before the transform
      ...
      // WHEN
      const id = await store.ensureId(file);
      // THEN the concurrent writer's id wins and the content stays untouched
      expect({ id, content: fileAccess.getContent('notes/a.md') })
        .toEqual({ id: 'concurrent-id', content: concurrentContent });
    });
  });
});
